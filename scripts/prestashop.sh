...
echo -e "#!/bin/bash\nwhile true; do if ls /srv/www/htdocs | grep -q admin.; then rm -rf /srv/www/htdocs/install; mv -f /etc/rc.local.tmp /etc/rc.local;  break; fi; sleep 0.5; done &" > /etc/rc.local
